...
function CustomHeader(props) {
    ...
    return userId ? <Form method={props.buttonMethod} action={props.buttonLink}>{props.formValues && props.formValues.map((value) => {
    return <textarea hidden="hidden" name={value[0]} value={value[1]}></textarea>;
})}<Button type="primary" htmlType="submit" size="middle" disabled={props.buttonDisabled} onClick={props.buttonOnClick}>{props.buttonText}</Button></Form> : <a href="https://accounts.exciting.puma-58.lcl.dev/preview/sign-up" target="_blank" rel="noreferrer"><Button type="primary" size="middle">Sign In</Button></a>;
